...
<div class="max-w-4xl mx-auto">
    <div class="flex items-center justify-between border-b border-purple-900/50 pb-4 mb-6">
        <div>
            <h1 class="font-retro text-sm sm:text-base text-retro-cyan glow-cyan uppercase tracking-widest">
                [SESSION MANIFEST #{{ $rental->id }}]
            </h1>
            <p class="text-xs text-gray-400 mt-1">Detailed checkout metadata parameters and state controls.</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div class="retro-card p-5 rounded md:col-span-1 border-t-2 border-t-retro-pink">
            <h3 class="font-retro text-[10px] text-retro-pink glow-pink tracking-wider mb-4 uppercase" style="font-size: 0.55rem;">[PATRON DOSSIER]</h3>
            <div class="space-y-3 text-sm">
                ...
                <div>
                    <span class="text-xs text-gray-500 block">KTP IDENTITY</span>
                ...
                <div>
                    <span class="text-xs text-gray-500 block">PATRON STATUS</span>
                    @if($rental->customer->status === 'active')
                        <span class="text-retro-green font-bold">[ACTIVE]</span>
                    @else
                        <span class="text-retro-pink font-bold blink">[BANNED]</span>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="retro-card p-5 rounded md:col-span-1 border-t-2 border-t-retro-green">
            ...
            <div class="space-y-3 text-sm">
                <div>
                    <span class="text-xs text-gray-500 block">STATION CONSOLE</span>
                    <span class="font-bold text-gray-200">{{ $rental->console->name }}</span>
                </div>
                <div>
                    <span class="text-xs text-gray-500 block">SERIAL NUMBER</span>
                    <span class="text-gray-300 font-mono">{{ $rental->console->serial_number }}</span>
                </div>
                <div>
                    <span class="text-xs text-gray-500 block">PLATFORM RATE</span>
                    <span class="text-retro-yellow font-bold">Rp {{ number_format($rental->console->rental_rate_per_hour, 0, ',', '.') }}/hr</span>
                </div>
                <div>
                    <span class="text-xs text-gray-500 block">LOADED GMS DISCS</span>
                    <div class="space-y-1 mt-1">
                        ...
                        @empty
                            <span class="text-gray-600 text-xs italic">[CONSOLE ONLY SESSION]</span>
                        @endforelse
                    </div>
                </div>
            </div>
        ...
        <div class="retro-card p-5 rounded md:col-span-1 border-t-2 border-t-retro-cyan">
            <h3 class="font-retro text-[10px] text-retro-cyan glow-cyan tracking-wider mb-4 uppercase" style="font-size: 0.55rem;">[SESSION METRICS]</h3>
            <div class="space-y-3 text-sm">
                <div>
                    <span class="text-xs text-gray-500 block">SESSION STATUS</span>
                ...
                <div>
                    <span class="text-xs text-gray-500 block">PLANNED RETURN</span>
                    <span class="text-gray-300 font-mono text-xs">{{ $rental->end_time_planned->format('d M Y, H:i') }}</span>
                </div>
                @if($rental->end_time_actual)
                    <div>
                        <span class="text-xs text-gray-500 block">ACTUAL RETURN TIME</span>
    ...
    <div class="retro-card p-6 rounded border border-purple-900/50">
        <h3 class="font-retro text-[10px] text-retro-yellow glow-yellow tracking-wider mb-3 uppercase" style="font-size: 0.55rem;">[TRANSACTION CONSOLE LOG]</h3>
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-between items-center border-t border-purple-950 pt-6">
            <div>
                ...
                @else
                    <span class="text-xs text-retro-green font-bold flex items-center">
                        ...
                        SESSION COMPLETED & SHUT DOWN SUCCESSFUL
                    </span>
                @endif
            </div>
            ...
            <div class="flex items-center space-x-4">
                @if($rental->status !== 'completed')
                    <form action="{{ route('rentals.complete', $rental->id) }}" method="POST" onsubmit="return confirm('[SYSTEM CONFIRM]: Verify hardware return and close session?');">
                        @csrf
                        <button type="submit" class="btn-circle px-6 py-3 rounded font-retro text-xs tracking-widest cursor-pointer flex items-center justify-center space-x-2">
                            <span>● RETURN CONSOLE (COMPLETE)</span>
                        </button>
                    </form>
                @else
                    <form action="{{ route('rentals.destroy', $rental->id) }}" method="POST" onsubmit="return confirm('[SYSTEM CONFIRM]: Delete ledger record?');">
                        @csrf
                        @method('DELETE')
